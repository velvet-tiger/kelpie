...
- **An SMTP provider** (Phase 4). `EMAIL_PROVIDER=log` is the only one core ships, which writes the message to the log instead of sending it, so this install cannot deliver an invite or a password reset. The integrations framework is not on this list: it is cloud, not core, per `modules.md`.
