 ...
 * companies and deals, because `api.md` has no include-expansion and no list
 * takes a set of bare ids. A target past those pages, or one whose type has no
 * page yet (opportunity, partnership, raise, candidate), names its record type
...
function useTargetDirectory(): TargetDirectory {
  ...
  const nameById = useMemo(
    () =>
      new Map(
        [...people.records, ...companies.records, ...deals.records].map((record) => [
          record.id,
          record.name,
        ]),
      ),
    [people.records, companies.records, deals.records],
  )

  return {
    nameFor: (decision) => nameById.get(decision.targetId),
    isComplete: !people.hasMore && !companies.hasMore && !deals.hasMore,
  }
}
