...
/**
 * Column conventions, in one place so tables cannot drift.
 *
 ...
 * the wire requires, so string mode would leak a non-conforming value straight
 ...
 *
 * Fixed enums are stored as text plus a check rather than as a Postgres enum,
 * so that adding a value is a constraint swap instead of a type migration.
