...
The endpoint takes **bearer keys only**, and deliberately not the session cookie the REST surface accepts. It sends no CORS headers either, and between them that is the whole answer to a cross-origin caller: a page on another origin can neither read a reply nor borrow a signed-in reader's identity, which is what the transport spec's DNS-rebinding warning is about. There is no session id and no server-initiated stream, so `GET` and `DELETE` answer `405` and any instance can answer any request. It speaks MCP `2025-06-18` and `2025-03-26`, and honours the latter's JSON-RPC batches.
