...
- **Agent tasks on the dashboard.** The mockup's daily brief carries an Agent tasks button, and the brief lines themselves are the `workspace.daily_brief` signals. The button arrives with the agent task registry in Phase 3, like every other record's.
- **The mockup's empty-field and pipeline-review brief lines.** Its brief also counts records missing a summary, an ICP fit or a thesis fit, and open pipeline records with no Plan. Those are the `workspace.empty_field_sweep` and `workspace.pipeline_review` signals in `agent-tasks.md` rather than this endpoint's, and they sweep five more tables; they belong to whichever feature builds those tasks.
